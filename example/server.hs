...
-- | We represent a client by his username and a 'WS.Sender'. We can use this
-- sender to asynchronously send 'Text' to the client later.
...
-- | Add a client (first, you should verify the client is not already connected
-- using 'clientExists')
...
main = do
    ...
    WS.runServer "0.0.0.0" 9160 $ do
        -- When a client connects, we first obtain the request. Once we have
        -- that, we use the 'WS.handshake' function to generate a response,
        -- which we finally send to the client.
        --
        -- We can use irrefutable pattern matches here because the server will
        -- spawn a thread for each connection -- and a crashing thread does not
        -- effect the rest of the program.
        Just rq <- WS.receiveRequest
        let Right rsp = WS.handshake rq
        WS.sendResponse rsp

        -- If we want to be able to send data to this client later, from another
        -- thread, we obtain a sender. We will add this to the server state
        -- later.
        sender <- WS.getSender
